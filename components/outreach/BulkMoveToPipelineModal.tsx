...
export default function BulkMoveToPipelineModal({ open, onClose, contacts, onMoved }: Props) {
  ...
  const eligible = contacts.filter((c) => c.status !== "in_pipeline");
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!stageId || eligible.length === 0) return;
    setSaving(true);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Move to Pipeline" className="max-w-lg">
      <form onSubmit={submit} className="space-y-5">
        {/* Summary */}
        <div style={{ padding: "12px 14px", background: "var(--cloud)", borderRadius: 8, border: "1px solid var(--mist)" }}>
          <p style={{ fontSize: 14, fontWeight: 700, color: "var(--ink)", margin: 0 }}>
            {eligible.length} contact{eligible.length !== 1 ? "s" : ""} will be added to the pipeline
          </p>
          {skipped > 0 && (
            <p style={{ fontSize: 12, color: "var(--stone)", marginTop: 4 }}>
              {skipped} already in pipeline — will be skipped
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3}
            placeholder="Context or conversation notes for all selected contacts"
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button type="submit" disabled={saving || !stageId || eligible.length === 0}
            style={{ height: 36, padding: "0 20px", background: "var(--ink)", color: "white", borderRadius: 999, border: "none", fontSize: 13, fontWeight: 500, cursor: "pointer", opacity: (saving || !stageId || eligible.length === 0) ? 0.5 : 1 }}>
            {saving ? "Moving…" : `Move ${eligible.length} to Pipeline`}
          </button>
        </div>
      </form>
    </Modal>
